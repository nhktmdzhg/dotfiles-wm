...
case "${@}" in
    "$A"     ) prompt "$A_" "${CMD} --no-ask-password poweroff"
    ;;
    "$B"     ) prompt "$B_" "${CMD} --no-ask-password reboot"
    ;;
    "$C"     ) prompt "$C_" '~/.config/awesome/xss-lock-tsl.sh'
    ;;
    "$D"     ) prompt "$D_" "${CMD} --no-ask-password suspend"
    ;;
    "$E"     ) prompt "$E_" "${CMD} --no-ask-password hibernate"
    ;;
